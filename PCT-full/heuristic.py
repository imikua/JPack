...
def MACS(env, times=2000):
        ...
        for h in range(H):
            ...
            for i in reversed(range(container_size[0])):
                for j in range(container_size[1]):
                    if i == container_size[0] - 1:
                        histmap[i, j] = hotmap[i, j]
                    elif hotmap[i, j] == 0:
                        histmap[i, j] = 0
                    else:
                        histmap[i, j] = histmap[i + 1, j] + hotmap[i, j]
    ...
    for counter in range(times):
        while True:
            ...
            bestScore = -1e10
            EMS = [x.node_vec for x in env.space.leaf_emss]
            ...
            if bestAction is not None:
                ...
                if args.draw:
                    placed_items = env.space.root_ems.getInternalNodes()
                    root_path = os.path.join('res_imgs', args.timeStr)
                    img_path = "rec" + str(len(placed_items)) + ".png"
                    tools.draw(placed_items, counter, img_path, root_path)
            else:
...
#
def minZ(env, times=2000, method = 'minZ'):
    done = False
    episode_utilization = []
    episode_length = []
    env.reset()
...
if __name__ == '__main__':
    ...
    if not args.without_time_str:
        ...
        args.timeStr = '{}_{}_{}_{}'.format(args.heuristic,
                                           'continuous' if args.continuous else 'discrete',
                                           args.sample_left_bound,
                                           args.sample_right_bound) + timeStr
    else:
        args.timeStr = args.custom
    ...
    if args.continuous == True:
        if args.env_version == 0:
            ...
            from pct_envs.PctContinuous2 import PackingContinuous
        PackingEnv = PackingContinuous
    else:
        if args.env_version == 0:
            from pct_envs.PctDiscrete0 import PackingDiscrete
        elif args.env_version == 2:
            from pct_envs.PctDiscrete2 import PackingDiscrete
        PackingEnv = PackingDiscrete

    env = PackingEnv(args = args)

    if args.heuristic == 'LSAH':
        mean, var, length = LSAH(env, args.evaluation_episodes)
    ...
    elif args.heuristic == 'minZ' or args.heuristic == 'DBL+':
        mean, var, length = minZ(env, args.evaluation_episodes, args.heuristic)
    else:
        raise ValueError('Unknown heuristic: {}'.format(args.heuristic))
    ...
    result_str = "The average space utilization: {}\n" \
                "The variance of space utilization: {}\n" \
                "The average number of packed items: {}\n".format(mean, var, length)
    targer_dir = os.path.join('./logs/evaluation', timeStr)
    if not os.path.exists(targer_dir):
        os.mkdir(targer_dir)
    file = open(os.path.join(targer_dir, 'result.txt'), 'w')
