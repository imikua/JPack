...
def quaternion_distance(q1, q2):
    ...
    q_diff = quaternion_multiply(q2_conjugate, q1)

    return quaternion_norm(q_diff)


class Interface:
    
    def __init__(self, bin = [10, 10, 5],
                 foldername = './datasets/datas/',
                 visual = False,
                 scale = [1.0,1.0,1.0],
    ...
    def addCube(self, cube_extents, targetFLB, density):
        cube_half_extents = np.array(cube_extents) / 2
        targetFLB = np.array(targetFLB)
        cube_collision_shape = p.createCollisionShape(p.GEOM_BOX, halfExtents=cube_half_extents, flags = 1)

        mass = np.prod(cube_extents) * density
        if self.visual:
        ...
        self.objs.append(cube_body)
        self.objsLocation[cube_body] = [targetFLB + cube_half_extents, cube_half_extents, np.array([0, 0, 0, 1])]
        # self.objsLocation[cube_body] = np.array(p.getAABB(cube_body))

        # self.massDict[cube_body] = np.prod(cube_extents) * density
        return cube_body

    def distance_to_registion(self):
        all_distances = []
        for box_id in self.objsLocation.keys():
            p.resetBaseVelocity(box_id, linearVelocity=[0, 0, 0], angularVelocity=[0, 0, 0])
            p.resetBasePositionAndOrientation(box_id, self.objsLocation[box_id][0], [0, 0, 0, 1])

        for _ in range(100):
            p.stepSimulation()

        for box_id in self.objsLocation.keys():
            registPosition  = self.objsLocation[box_id]
            currentPosition = np.array(p.getAABB(box_id))
            distance = np.linalg.norm(registPosition[0] - currentPosition[0] - self.objsLocation[box_id][1])
            distance /= np.max(self.bin)
    ...
    def enableAllObjects(self):
        for id in self.objs:
            p.changeDynamics(id, -1, self.massDict[id])
            self.objsDynamic.append(id)
