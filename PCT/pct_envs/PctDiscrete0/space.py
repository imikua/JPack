...
class Box(object):
    ...
    def calculate_new_com(self, virtual=False):
        ...
        # Degenerate contact geometry can yield zero total support mass. Keep the
        # current box center instead of injecting inf/nan into later physics checks.
        if abs(new_stack_mass) <= 1e-12:
            new_stack_centre = self.centre.copy()
            new_stack_mass = 0.0
        else:
...
class Space(object):
    def __init__(self, width=10, length=10, height=10, size_minimum=0, holder = 60):
        ...
        self.holder = holder  # 保存容量限制，防止越界
