...
class KFACOptimizer(object):
    ...
    optimizer with lr=1.0, momentum=0.0:
      ...
      2) probe_optim.backward(loss) → computes gradients via Jittor's native path
      3) grad = p.opt_grad(probe_optim)
      4) Apply KFAC preconditioning on extracted grad
      5) Update params with momentum (all in numpy)

    For Fisher stats (acc_stats=True), we use jt.grad on module OUTPUTS (shorter
    graph, more reliable than gradients w.r.t. all parameters).
    """
    ...
    def __init__(self, model, lr=0.25, momentum=0.9, stat_decay=0.99,
        ...
        for module in self.model.modules():
            ...
            if classname in self.known_modules:
                if classname in ['Linear', 'Conv2d'] and module.bias is not None:
                    # Unsplit layers (skipped by _should_skip_split_bias) are excluded
                    # from KFAC tracking; they still receive raw first-order updates.
                    continue
        ...
        self.debug = os.environ.get('PCT_DEBUG_ACKTR', '0') == '1'
        self.debug_interval = int(os.environ.get('PCT_DEBUG_ACKTR_INTERVAL', '50'))
    ...
    def _update_output_grad_stats(self, loss):
        ...
        # jt.grad on module outputs (short graph path) is reliable in Jittor.
        # retain_graph=True because the same graph is still referenced by _outputs.
    ...
    def backward_and_step(self, loss, retain_graph=False, max_grad_norm=None):
        ...
        Uses probe_optim.backward(loss) + p.opt_grad() to extract gradients
        (the official Jittor way). All preconditioning and parameter updates
        done in numpy to prevent graph accumulation.
        """

        if self.acc_stats:
            # CRITICAL ORDER: m_gg first (graph intact), then m_aa (.stop_grad() sync).
        ...
        # Recreate a probe optimizer every step to avoid stale internal state.
        ...
        if self.debug and (self.steps <= 10 or self.steps % self.debug_interval == 0):
        ...
        for m in self.modules:
            ...
            if self.steps % self.Tf == 0 and m in self.m_gg and m in self.m_aa:
                ...
                # Symmetrize to handle tiny floating-point asymmetry
                gg_data = 0.5 * (gg_data + gg_data.T)
                aa_data = 0.5 * (aa_data + aa_data.T)
        ...
        # ---- Phase 6: SGD with momentum + parameter update (all numpy) ----
        ...
        for param_idx, p in enumerate(all_params):
            ...
            delta_stats.append(float(_np.max(_np.abs(new_p - saved_np[pid]))))
        ...
        if self.debug and (self.steps <= 10 or self.steps % self.debug_interval == 0):
            stat_module_count = sum(1 for m in self.modules if m in self.m_aa and m in self.m_gg)
            max_delta = max(delta_stats) if delta_stats else 0.0
            mean_delta = float(_np.mean(delta_stats)) if delta_stats else 0.0
            print(
                "[debug acktr] "
                f"step={self.steps} "
                f"captured_modules={len(self.modules)} "
                f"stat_modules={stat_module_count} "
                f"vg_sum={vg_sum:.6e} "
                f"nu={nu:.6e} "
