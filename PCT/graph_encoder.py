...
class MultiHeadAttention(nn.Module):
    ...
    def init_parameters(self):
        ...
        for param in self.parameters():
            ...
            param.assign(jt.array(np.random.uniform(-stdv, stdv, param.shape).astype(np.float32)))
    ...
    def execute(self, data, h=None):
        ...
        compatibility = self.norm_factor * jt.matmul(Q, K.transpose(2, 3))

        # Optionally apply mask to prevent attention
        mask = mask.unsqueeze(1).repeat(1, graph_size, 1).bool()
        if mask is not None:
            mask = mask.view(1, batch_size, n_query, graph_size).expand(compatibility.shape)
            if data['evaluate']:
                compatibility = jt.where(mask, jt.array(-math.inf).broadcast(compatibility.shape), compatibility)
            else:
                compatibility = jt.where(mask, jt.array(-30.0).broadcast(compatibility.shape), compatibility)
        attn = nn.softmax(compatibility, dim=-1)

        # If there are nodes with no neighbours then softmax returns nan so we fix them to 0
        if mask is not None:
            attn = jt.where(mask, jt.zeros(attn.shape), attn)
...
class GraphAttentionEncoder(nn.Module):
    ...
    def execute(self, x, mask=None, limited=False, evaluate = False):
        ...
        return (h, h.view(int(h.shape[0] / self.graph_size), self.graph_size, -1).mean(dim=1),)
