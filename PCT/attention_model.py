...
from tools import observation_decode_leaf_node, init, orthogonal_init
...
class AttentionModelFixed:
    ...
    def __getitem__(self, key):
        if isinstance(key, jt.Var) or isinstance(key, slice):
...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        init_ = lambda m: init(m, orthogonal_init, lambda x: nn.init.constant_(x, 0), nn.init.calculate_gain(ini))
    ...
    def execute(self, input, deterministic = False, evaluate_action = False, normFactor = 1, evaluate = False):
        ...
        init_embedding = jt.cat((internal_embedded_inputs, leaf_embedded_inputs, next_embedded_inputs), dim=1).view(batch_size * graph_size, self.embedding_dim)
    ...
    def _inner(self, embeddings, mask = None, deterministic = False, evaluate_action = False, shape = None, full_mask = None, valid_length =None): # 元素齐了
        ...
        if deterministic:
            ...
            if jt.sum(masked_outs) == 0:
                masked_outs += 1e-20
        else:
            masked_outs = log_p * (1 - mask) + 1e-20
        denom = jt.maximum(jt.sum(masked_outs, dim=1).unsqueeze(1), jt.full((masked_outs.shape[0], 1), 1e-20, dtype=masked_outs.dtype))
        log_p = masked_outs / denom
    ...
    def _precompute(self, embeddings, num_steps=1, shape = None, full_mask = None, valid_length = None):
        ...
        transEmbedding = jt.where(full_mask, jt.zeros(transEmbedding.shape), transEmbedding)
        graph_embed = transEmbedding.view(shape).sum(1)
        transEmbedding = transEmbedding.view(embeddings.shape)

        safe_valid_length = jt.maximum(valid_length.reshape((-1, 1)), jt.ones((shape[0], 1), dtype=graph_embed.dtype))
        graph_embed = graph_embed / safe_valid_length
    ...
    def _make_heads(self, v, num_steps=None):
        ...
        return (
            v.contiguous().view(v.shape[0], v.shape[1], v.shape[2], self.n_heads, -1)
