...
class TAP(gym.Env):
    ...
    def generate_new_task(self):
        ...
        # time_series
        file_path = "/home/wzf/Workspace/rl/RobotPackingBenchmark-main-v4-pat/packing_dataset/宝洁-Simulation+data.xlsx"
        df = pd.read_excel(file_path, engine='openpyxl')
        # Extract the relevant columns
        init_sizes = df[['Length_cm', 'Width_cm', 'Height_cm']].to_numpy() / 100  # 将单位由cm转换为m
        init_sizes = init_sizes / 2  # 此处缩小2倍方便packing验证



        # semantic
        # file_path = '/home/wzf/Workspace/rl/RobotPackingBenchmark-main-v4-pat/packing_dataset/得力选品desk.xlsx'
        # df = pd.read_excel(file_path, sheet_name='选品', engine='openpyxl')
        # # Extract the relevant columns
        ...
        # flat_long
        # folder_path = '/home/wzf/Workspace/rl/RobotPackingBenchmark-main-v4-pat/packing_dataset/欧派家居opai'
        # # 初始化一个集合来存储去重后的数据
        # unique_data = set()
        # # 读取文件夹内的每个txt文件
        ...
        #                 if np.round(np.array((data_tuple[0] / 100) / 4) / self.block_unit).astype(
        #                         np.int) > 15 or np.round(np.array((data_tuple[1] / 100) / 4) / self.block_unit).astype(
        #                         np.int) > 15:
        #                     continue
        #                 unique_data.add(data_tuple)
        # # 将集合转换为numpy数组
        # sizes = np.array(list(unique_data)) / 100  # 将单位由cm转换为m
        # sizes = sizes / 4  # 此处缩小四倍方便packing验证
